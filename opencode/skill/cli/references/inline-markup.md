...
cli_abort(c(
  ...
  "x" = "Set {.arg na.rm = TRUE} to handle missing values"
))
```
...
- `.path` - Directories, installation locations
```
...
cli_text("Read {.href [vignette:cli::inline-markup]} for examples")
...
cli_text("Read {.help topic 'Error Handling'}")
```
...
cli_inform("Refer to {.topic base::sum}")
```
...
```r
cli_text("Read {.vignette cli::inline-markup} for examples")
cli_text("Tutorial: {.vignette dplyr::programming}")
```
...
```r
cli_text("Press {.kbd Ctrl+C} to copy")
cli_text("Save with {.kbd Cmd+S} (Mac) or {.kbd Ctrl+S} (Windows)")
```
...
- Value entries in key-value pairs
...
#> Variables: a, b, c, d, e, f, and 19 more
...
status <- "complete"
cli_text("Status: {.toupper(status)}")
...
type <- class(x)
cli_text("Type: {if(is.numeric(x)) .code else .cls {class(x)}}")
```
...
### Three Alternative Pattern (zero/one/many)

Handle zero/one/many:

```r
nfile <- 0
cli_text("{?No/One/Some} file{?s}: {?no/the/the/the} file{?s}")
...
nupd <- 3
ntotal <- 10
cli_text("{nupd}/{ntotal} {qty(nupd)} file{?s} {?needs/need} update{?s}")
...
for (i in 1:10000) {
  cli_alert("Processing {.val {i}}")
}
```

# Better - batch messages
if (i %% 1000 == 0) cli_alert("Processing {.val {i}}")
```
...
```r
# Inefficient
if (verbose) cli_alert("Status: {.val {expensive_status_check()}")
```

# Better
if (verbose) {
  status <- expensive_status_check()
  cli_alert("Status: {.val status}")
}
```
