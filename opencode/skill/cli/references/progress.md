...
- Output shows progress bar with spinner and percentage
...
cli_progress_bar(
  format = "Working {cli::pb_bar} {cli::pb_percent} | ETA: {cli::pb_eta}",
  total = 100,
  type = "custom"
)
```
...
# With elapsed time
cli_progress_bar(
  format = "Working: {cli::pb_name} {cli::pb_elapsed}"
)
```
...
cli_progress_bar(
  "Processing",
  total = 100,
  format = "Working: {cli::pb_name} {cli::pb_percent}",
...
for (i in 1:50) {
  if (i == 30) {
    cli_progress_done(result = "failed", msg_failed = "Processing failed at item 30")
    break
  }
  cli_progress_update()
}
...
Classic style with equals sign:
...
Note: C-level progress requires cli package C headers and proper linking.
...
# Better: Periodic updates
cli_progress_bar("Processing", total = 1e6)
for (i in 1:1e6) {
  fast_operation(i)
  if (i %% 1000 == 0) cli_progress_update(inc = 1000)
}
}
```
