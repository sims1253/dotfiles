...
```r
nfile <- 0
cli_text("Found {?no/the/the/the} file{?s}: {?no/the/the/the} file{?s}")
#> Found 0 files: no files

nfile <- 1
cli_text("Found {?no/One/Some} file{?s}: {?no/the/the/the} file{?s}")
...
# After (cli)
cli_warn("Column {.field {col}} has {n_missing} missing value{?s}")
