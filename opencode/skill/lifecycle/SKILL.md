...
| ------------------ | ------------------------------------------------------- |
| `deprecate_soft()` | First stage; warns only:
   - Users calling from global environment
   - Developers running tests
   - Does NOT warn when called indirectly by another package |

| `deprecate_warn()` | Standard deprecation; warns all users once per 8 hours |

| `deprecate_stop()` | Final stage before removal; errors with helpful message about replacement |
...
#' `df %>% pivot_longer(c(x, y, z), names_to = c("key", "value"))`
```
```
